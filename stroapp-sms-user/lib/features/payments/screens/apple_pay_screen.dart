...
class _ApplePayScreenState extends ConsumerState<ApplePayScreen> {
  final _mockToken = 'mock-apple-pay-token-${DateTime.now().millisecondsSinceEpoch}';
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(paymentsProvider.notifier).fetchProducts('apple');
    });
  }
  ...
  Widget _buildBody(PaymentsState state) {
    ...
    if (state.error != null && state.products.isEmpty) {
      return CustomErrorWidget(
        message: state.error!,
        onRetry: () => ref.read(paymentsProvider.notifier).fetchProducts('apple'),
      );
    }
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: Icon(QasehIcons.buy_filled, size: 24, color: AppColors.primary),
                        ),
  ...
  Future<void> _handlePayment() async {
    ...
    try {
      final idempotencyKey = 'ap-${DateTime.now().millisecondsSinceEpoch}';
      await ref.read(paymentsApiProvider).applePay(_mockToken, _selectedProductId!, idempotencyKey);
      if (mounted) {
